...
# Every AINL program has:
#   - A header line   S <scope> <adapter> <path>  (declares the "surface")
#   - Labels          L1: L2: ... END  (control-flow nodes)
#   - Steps inside:
#       R <adapter>.<op> [args...] -><var>   Request (side-effectful call)
#       X <var> <value>                      Assign a local variable
#       J <var>                              Join / return a value
#       If (<expr>) -><then-label> -><else-label>   Branch
#
# This demo program adds two numbers, formats a greeting, and returns it.

S app core noop

L1:
  # Add two numbers and bind the result to `sum`
  R core.ADD 2 3 ->sum
  J sum

L2:
  # Assign a greeting string
  X greeting "Hello from AINL! Your first program is working."
  J greeting

# END is the implicit terminal label; explicit jumps land here on success.
'''
