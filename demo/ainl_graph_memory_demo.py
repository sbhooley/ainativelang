...
Usage (from ainativelang repo root):
    python3 demo/ainl_graph_memory_demo.py

Requirements:
    This repository (``ainativelang``): Python 3.10+, stdlib, plus ``armaraos/bridge`` and
    ``runtime`` import paths as configured below (no separate ``ainl-memory`` wheel required).
"""
...
# Script lives under demo/; repo root is one level up.
