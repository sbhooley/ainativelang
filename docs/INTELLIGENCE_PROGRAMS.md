...
**Stateful graphs:** optional **`~/.armaraos/agents/<agent_id>/bundle.ainlbundle`** plus **`AINL_BUNDLE_PATH`** / post-run export (ArmaraOS kernel) let **`ainl_graph_memory`** persona snapshots persist across scheduled runs; see **armaraos** [`docs/scheduled-ainl.md`](https://github.com/sbhooley/armaraos/blob/main/docs/scheduled-ainl.md) (*AINL bundle + graph memory*). With **`ARMARAOS_AGENT_ID`** set, Python can also append graph rows to **`ainl_graph_memory_inbox.json`** for Rust ingest — **ainativelang** [`armaraos/docs/graph-memory-sync.md`](../armaraos/docs/graph-memory-sync.md). That is separate from Rust **`ainl_memory.db`** persona lines injected into **dashboard chat** system prompts — **armaraos** [`docs/graph-memory.md`](https://github.com/sbhooley/armaraos/blob/main/docs/graph-memory.md).
