...
**Current `ainl` release:** **v1.5.2** — [`CHANGELOG.md`](CHANGELOG.md), [`RELEASE_NOTES.md`](RELEASE_NOTES.md) (PyPI / **`RUNTIME_VERSION`** / citation metadata).
...
- **Long-form architecture / economics:** **[`WHITEPAPERDRAFT.md`](../WHITEPAPERDRAFT.md)** (repo root) — through **v1.5.2** (native Solana + OpenClaw intelligence ops, token caps, Hermes Agent, ArmaraOS host pack, graph-memory IR + **`.ainlbundle`**, `ainl install openclaw`, `ainl status`).
