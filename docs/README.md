...
- **Long-form architecture / economics:** **[`WHITEPAPERDRAFT.md`](../WHITEPAPERDRAFT.md)** (repo root) — v1.2.8/v1.3.0 — OpenClaw intelligence ops, token caps, Hermes Agent support, ainl install openclaw, ainl status.
...
- **Tiered code context (optional):** [`adapters/CODE_CONTEXT.md`](adapters/CODE_CONTEXT.md) — **`code_context`** adapter, demo [`../examples/code_context_demo.ainl`](../examples/code_context_demo.ainl), `--enable-adapter code_context` (not part of the default Hyperspace emitted agent; enable at runtime when needed).
