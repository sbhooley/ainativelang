...
**Current `ainl` release:** **v1.5.1** — [`CHANGELOG.md`](CHANGELOG.md), [`RELEASE_NOTES.md`](RELEASE_NOTES.md) (PyPI / **`RUNTIME_VERSION`** / citation metadata).
...
- [`adapters/`](adapters/README.md) — adapters, capabilities, and host integration surfaces (includes **[`adapters/AINL_GRAPH_MEMORY.md`](adapters/AINL_GRAPH_MEMORY.md)** for **`ainl_graph_memory`** + **`MemoryRecall`/`MemorySearch`**)
...
- **Long-form architecture / economics:** **[`WHITEPAPERDRAFT.md`](../WHITEPAPERDRAFT.md)** (repo root) — through **v1.5.1** (native Solana + OpenClaw intelligence ops, token caps, Hermes Agent, ArmaraOS host pack, `ainl install openclaw`, `ainl status`).
