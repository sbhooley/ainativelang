...
- **A2A adapter (Agent-to-Agent, ArmaraOS wire):** **[`integrations/A2A_ADAPTER.md`](integrations/A2A_ADAPTER.md)** — threat model, `strict_ssrf`, redirects, reporting via **[`../SECURITY.md`](../SECURITY.md)**
...
- **Audit and telemetry map (single inventory):** [`operations/AUDIT_AND_TELEMETRY_MAP.md`](operations/AUDIT_AND_TELEMETRY_MAP.md) — runner HTTP audit vs CLI trajectory vs `audit_trail` adapter vs observability JSONL; links to `STATUS.yaml` and enterprise mapping docs.
- **Enterprise shared responsibility + evidence bundle recipe:** [`enterprise/SHARED_RESPONSIBILITY.md`](enterprise/SHARED_RESPONSIBILITY.md), [`enterprise/EVIDENCE_BUNDLE_RECIPE.md`](enterprise/EVIDENCE_BUNDLE_RECIPE.md), index [`enterprise/README.md`](enterprise/README.md).
