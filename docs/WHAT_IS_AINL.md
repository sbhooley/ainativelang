...
- **Optional tiered code context:** **`code_context`** indexes a repo to JSON and serves ctxzip-style tiers (TF–IDF summaries by default); enable with **`--enable-adapter code_context`**. Guide: **`docs/adapters/CODE_CONTEXT.md`**.
...
| OpenClaw gold standard + host briefing (v1.3.0) | `docs/operations/OPENCLAW_AINL_GOLD_STANDARD.md`, `docs/operations/OPENCLAW_HOST_AINL_1_2_8.md` |
...
Long-form architecture: [`WHITEPAPERDRAFT.md`](../WHITEPAPERDRAFT.md) (repository root) — includes **v1.3.0** positioning on OpenClaw intelligence, token caps, embedding pilot, graph-runtime pitfalls, and the weekly cap auto-tuner.
