...
| Graph memory **inbox** (Python → Rust ingest) | `ARMARAOS_AGENT_ID` | Required for **`AinlMemorySyncWriter`** / bridge **`_sync`** pushes; with **`ARMARAOS_HOME`** (or **`OPENFANG_HOME`**, or default **`~/.armaraos`**) and existing **`agents/`** dir, appends to **`agents/<id>/ainl_graph_memory_inbox.json`**. See **`armaraos/docs/graph-memory-sync.md`**. |
