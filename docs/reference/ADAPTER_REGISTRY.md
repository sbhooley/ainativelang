...
- **`ainl_graph_memory`** (tier: `extension_openclaw`, lane: `noncanonical`): JSON-backed **graph** memory for bridge workflows — strict verbs **`MEMORY_RECALL`**, **`MEMORY_SEARCH`**, **`MEMORY_PATCH`** (runtime dispatches IR **`MemoryRecall`** / **`MemorySearch`** and GraphPatch **`R memory.patch`** via **`adapters.call("ainl_graph_memory", "graph_patch", [memory_node_id, label_name], …)`**; bridge also accepts **`memory_patch`** as an alias). Reference implementation and ArmaraOS wiring: `armaraos/bridge/ainl_graph_memory.py`. Env: **`AINL_GRAPH_MEMORY_PATH`**; optional **inbox write-back** to ArmaraOS SQLite via **`ARMARAOS_AGENT_ID`** / **`ARMARAOS_HOME`** → **`ainl_graph_memory_inbox.json`** (**`armaraos/bridge/ainl_memory_sync.py`**, **`armaraos/docs/graph-memory-sync.md`**). Contract: [`../adapters/AINL_GRAPH_MEMORY.md`](../adapters/AINL_GRAPH_MEMORY.md).
...
- **`code_context`** (tier: `compatibility`, lane: `noncanonical`): tiered codebase index (ctxzip-style) plus import-graph **`GET_DEPENDENCIES`**, reverse impact + PageRank **`GET_IMPACT`**, and TF–IDF greedy token packing **`COMPRESS_CONTEXT`** — full verb set: **`INDEX`**, **`QUERY_CONTEXT`**, **`GET_FULL_SOURCE`**, **`GET_DEPENDENCIES`**, **`GET_IMPACT`**, **`COMPRESS_CONTEXT`**, **`STATS`**. Implementation: `adapters/code_context.py`. Env: **`AINL_CODE_CONTEXT_STORE`** (default `.ainl_code_context.json` in cwd). Enable: **`--enable-adapter code_context`** on **`ainl run`** (MCP install alone does not enable it). Guide: [`../adapters/CODE_CONTEXT.md`](../adapters/CODE_CONTEXT.md). Tiered design: [BradyD2003/ctxzip](https://github.com/BradyD2003/ctxzip), Brady Drexler. Graph/impact/packing ideas: [chrismicah/forgeindex](https://github.com/chrismicah/forgeindex), Chris Micah.
...
- **`ptc_runner`** (tier: `compatibility`, lane: `noncanonical`): optional external PTC Runner bridge for PTC-Lisp execution. Verbs: **`RUN`** (supports both **`R ptc_runner run ...`** and **`R ptc_runner.RUN ...`**). Returns envelope **`{ok, runtime, status_code?, result, traces}`**. Implementation: `adapters/ptc_runner.py`. Enable with **`--enable-adapter ptc_runner`** or **`AINL_ENABLE_PTC=true`**. Config/env: **`AINL_PTC_RUNNER_URL`**, **`AINL_PTC_RUNNER_MOCK`**, **`AINL_PTC_RUNNER_CMD`**. Full guide: `docs/adapters/PTC_RUNNER.md`.
- **`llm_query`** (tier: `compatibility`, lane: `noncanonical`): optional ad-hoc LLM query bridge. Verbs: **`QUERY`**, **`RUN`** (supports split and dotted forms, e.g. **`R llm_query query ...`**, **`R llm_query.QUERY ...`**). Returns envelope **`{ok, runtime, status_code?, result, traces}`**. Implementation: `adapters/llm_query.py`. Enable with **`--enable-adapter llm_query`** or **`AINL_ENABLE_LLM_QUERY=true`**. Config/env: **`AINL_LLM_QUERY_URL`**, **`AINL_LLM_QUERY_MOCK`**, **`AINL_LLM_QUERY_CMD`**.
