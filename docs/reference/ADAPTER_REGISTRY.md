...
- **`ainl_graph_memory`** (tier: `extension_openclaw`, lane: `noncanonical`): JSON-backed **graph** memory for bridge workflows — strict verbs **`MEMORY_RECALL`**, **`MEMORY_SEARCH`** (runtime also dispatches IR **`MemoryRecall`** / **`MemorySearch`** to `adapters.call("ainl_graph_memory", ...)`). Reference implementation and ArmaraOS wiring: `armaraos/bridge/ainl_graph_memory.py`. Env: **`AINL_GRAPH_MEMORY_PATH`**. Contract: [`../adapters/AINL_GRAPH_MEMORY.md`](../adapters/AINL_GRAPH_MEMORY.md).
