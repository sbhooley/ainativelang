...
- **`code_context`** (tier: `compatibility`, lane: `noncanonical`): tiered codebase index (ctxzip-style) — verbs **`INDEX`**, **`QUERY_CONTEXT`**, **`GET_FULL_SOURCE`**, **`STATS`**. Implementation: `adapters/code_context.py`. Env: **`AINL_CODE_CONTEXT_STORE`** (default `.ainl_code_context.json` in cwd). Enable: `--enable-adapter code_context`. Guide: [`../adapters/CODE_CONTEXT.md`](../adapters/CODE_CONTEXT.md). Concept credit: [BradyD2003/ctxzip](https://github.com/BradyD2003/ctxzip), Brady Drexler.
