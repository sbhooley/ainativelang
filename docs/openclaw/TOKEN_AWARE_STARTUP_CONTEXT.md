...
This reduces session bootstrap tokens from ~3,200 (full MEMORY.md) to ~150 tokens (>95% reduction), preventing context max-outs during high-frequency usage. It is part of the AINL v1.2.8+ enhancements for OpenClaw (shipped; current **v1.3.1** includes `ainl install openclaw` and `ainl status`).
...
- **Optimizations (v1.2.8–v1.3.1 cumulative):**
