...
> **Tier:** 🟢 **Core** · **Group:** `memory` · **Verbs:** `put`, `get`, `append`, `list`, … (6 total) · **Privilege:** — · **Sandbox-safe:** —
>
> Production-grade adapters on the gold path. Tier model and full adapter catalog: [`ADAPTER_TIERS.md`](ADAPTER_TIERS.md).
<!-- adapter-badge:end -->
