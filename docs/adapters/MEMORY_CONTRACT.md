...
**ArmaraOS JSON graph memory (bridge, separate adapter):** typed nodes/edges in **`~/.armaraos/ainl_graph_memory.json`** (or **`AINL_GRAPH_MEMORY_PATH`**), adapter name **`ainl_graph_memory`**, plus runtime IR ops **`MemoryRecall`** / **`MemorySearch`**, typed **`EdgeType`** relations (structural + epistemic), and label / engine integration for **`persona.update`** — see **[`AINL_GRAPH_MEMORY.md`](AINL_GRAPH_MEMORY.md)**. Optional **Python→Rust inbox** (**`ARMARAOS_AGENT_ID`**, **`ainl_graph_memory_inbox.json`**) — **[`../armaraos/docs/graph-memory-sync.md`](../armaraos/docs/graph-memory-sync.md)**.
