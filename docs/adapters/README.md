...
- [`AINL_GRAPH_MEMORY.md`](AINL_GRAPH_MEMORY.md) — **`ainl_graph_memory`** (JSON graph store, ArmaraOS bridge), IR ops **`MemoryRecall`** / **`MemorySearch`**, **`EdgeType`** (structural + epistemic edges), **`persona.update`** → bridge **`persona_update`**, optional graph viz server; **`.ainlbundle` boot** (**`AINL_BUNDLE_PATH`**) restores **`persona`** + **`memory`** (non-persona nodes) before scheduled **`ainl run`**; **Python inbox:** **`armaraos/bridge/ainl_memory_sync.py`**, **`armaraos/docs/graph-memory-sync.md`** (**`ARMARAOS_AGENT_ID`**, **`ainl_graph_memory_inbox.json`**); **Python hosts:** **`adapters/armaraos_integration.py`** (**`build_armaraos_monitor_registry`**, **`boot_armaraos_graph_memory`**, pre-seeded **`bridge`** / **`cron_drift_check`**); runtime **`AdapterRegistry.get()`** for registered instances; demos **`demo/procedural_roundtrip_demo.py`**, **`demo/ainl_graph_memory_demo.py`** · tests **`tests/test_semantic_edges.py`**, **`tests/test_armaraos_monitor_registry.py`**, **`armaraos/bridge/tests/test_ainl_memory_sync.py`** · narrative + timeline: **[ainativelang.com — graph-as-memory](https://ainativelang.com/blog/graph-as-memory-architecture-ainl)**, **[`PRIOR_ART.md`](../../PRIOR_ART.md)**
