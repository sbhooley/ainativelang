...
| **T2.3** | `scripts/benchmark_vs_hand_runner.py` — port `enterprise_monitor.ainl` to ~200 LOC Python runner; measure (a) source tokens, (b) audit-trail completeness, (c) lift-to-Temporal LOC, (d) compile-time error catch rate when an LLM authors the source | 🔴 | All five axes from `VS_HAND_WRITTEN_RUNNER.md` table become measured numbers; doc updated |
