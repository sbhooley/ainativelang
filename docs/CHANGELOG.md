...
- **feat(memory / graph IR)**: optional **`memory_type`** on per-label **`R`** nodes; **`memory.pattern_recall`** on the graph bridge + **`__last_pattern__`** for **`memory.merge`**; per-label **`emit_edges`** (**`port: "data"`** / **`"emit"`**) and **`tooling.graph_api`** helpers **`emit_edges`**, **`data_flow_edges`**, **`memory_nodes`** (**`label_*`** accept label id sequences). Docs: **`GRAPH_SCHEMA.md`**, **`GRAPH_INTROSPECTION.md`**, **`AINL_GRAPH_MEMORY.md`**, **`MEMORY_CONTRACT.md`**.
