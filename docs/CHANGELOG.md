...
- **feat(adapters)**: optional **`code_context`** adapter (`adapters/code_context.py`) — tiered codebase context (ctxzip-style): JSON index, TF–IDF Tier 1, verbs `INDEX` / `QUERY_CONTEXT` / `GET_FULL_SOURCE` / `STATS`; enable with `--enable-adapter code_context`; see `docs/adapters/CODE_CONTEXT.md`, `examples/code_context_demo.ainl`, and README. Concept credit: [BradyD2003/ctxzip](https://github.com/BradyD2003/ctxzip), **Brady Drexler**.
- **feat(code_context)**: import-based **dependency graph** on indexed chunks, **`GET_DEPENDENCIES`** / **`GET_IMPACT`** (transitive importers + global PageRank), and **`COMPRESS_CONTEXT`** (TF–IDF order, greedy token-budget packing); docs, strict **`tooling/effect_analysis.py`** + **`tooling/adapter_manifest.json`**, curriculum/training pack refresh for `examples/code_context_demo.ainl`. Graph/impact/packing ideas: [chrismicah/forgeindex](https://github.com/chrismicah/forgeindex), **Chris Micah** (alongside ctxzip credit above).
