...
- **Machine-readable bootstrap:** `tooling/bot_bootstrap.json` — pointers to onboarding doc, preflight doc, safe-default docs, advanced docs, required steps, and **`openclaw_commands`** (v1.3.3 schema **`version`**; OpenClaw CLI commands since v1.3.0: `ainl_install_openclaw`, `ainl_status`, `ainl_doctor_ainl`, `ainl_install_mcp`).
...
- **OpenClaw host briefing — AINL v1.2.8–v1.6.0 (what ships vs host obligations; current PyPI v1.6.0):** `docs/operations/OPENCLAW_HOST_AINL_1_2_8.md` — **`openclaw_host_ainl_1_2_8`**
...
- Contributor entrypoint: `CONTRIBUTING.md` (canonical) · docs hub: `docs/CONTRIBUTING.md` (v1.3.3 pointers)
...
- **Whitepaper draft (long-form architecture, OpenClaw + token economics through v1.3.3, including native Solana):** **`WHITEPAPERDRAFT.md`** (repository root)
...
- **ArmaraOS + bootstrap (`ainl install armaraos`, `~/.armaraos/config.toml`, `~/.armaraos/bin/ainl-run`):** `docs/ARMARAOS_INTEGRATION.md` — includes **`AINL_BUNDLE_PATH`** / bundle **`.ainlbundle`** for scheduled **`ainl run`**; host narrative: **armaraos** [`docs/scheduled-ainl.md`](https://github.com/sbhooley/armaraos/blob/main/docs/scheduled-ainl.md). (PostHog for the **desktop app** is configured on the **armaraos** repo / marketing site env — not in this compiler repo.)
- **ArmaraOS JSON graph memory (`ainl_graph_memory`, `MemoryRecall`/`MemorySearch`, typed `EdgeType` + `persona.update`, viz server):** `docs/adapters/AINL_GRAPH_MEMORY.md` · SQLite procedural **MemoryMerge** (`memory.merge`, `store_pattern`/`recall_pattern`): `docs/adapters/MEMORY_CONTRACT.md` §3.7 · demos `demo/procedural_roundtrip_demo.py`, `demo/ainl_graph_memory_demo.py` · tests `tests/test_memory_recall_op.py`, `tests/test_memory_search_op.py`, `tests/test_memory_merge.py`, `tests/test_semantic_edges.py`
...
- Memory contract and v1 adapter: `docs/adapters/MEMORY_CONTRACT.md`
  ...
  - Opt-in **`last_accessed` / `access_count`** helpers (include module; graph-safe list path **`LACCESS_LIST_SAFE`**): `modules/common/access_aware_memory.ainl`, `modules/common/README.md`, `docs/RELEASE_NOTES.md` (**shipped v1.2.4**; **latest release v1.3.3**)
