...
remove_legacy_armaraos_bin_ainl_shim() {
    local home="$1"
    ...
    if ! is_armaraos_bin_ainl_cron_shim "$shim"; then
        return 0
    fi
    if [ -e "$backup" ]; then
    ...
    mv "$shim" "$backup"
    echo "  Removed legacy ~/.armaraos/bin/ainl cron shim (saved as $(basename "$backup"))"
}

resolve_repair_ainl_executable() {
    local home="$1"
    ...
    if [ -f "$cache_line" ]; then
        ...
        if [ -n "$cached" ] && [ -x "$cached" ] && ! is_armaraos_bin_ainl_cron_shim "$cached"; then
            echo "$cached"
            return 0
        fi
    fi
...
repair_ainl_layout() {
    ...
    echo "  Refreshing AINL MCP registration..."
