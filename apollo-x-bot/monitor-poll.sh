#!/usr/bin/env bash
# Optional: snapshot monitored_accounts KV + audit (follow_manager.ainl). Gateway must be running.
set -euo pipefail
...
cd "$ROOT"
exec "$PY" -m cli.main run "$BOT_DIR/modules/apollo/follow_manager.ainl" --strict --label _monitor_list \
