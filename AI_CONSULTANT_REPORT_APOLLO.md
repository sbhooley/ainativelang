...
- **Token Cost Efficiency:** AINL delivers dramatic prompt/completion compression. Complex monitors that require 50–70k tokens in AINL (including the program itself and runtime data) would require 3–5× more tokens in Python/TypeScript when generated by an LLM, and they would lack strict validation, graph introspection, and multi-target emission. After the initial learning curve, AINL reduces per-task token burn by 2–5× for non-trivial automation. For simple tasks, savings are smaller but still positive due to DSL density. Bottom line: AINL lowers overall token usage while increasing capability and reliability.
...
| Standardized health envelope spec | `docs/STANDARDIZED_HEALTH_ENVELOPE.md` |
