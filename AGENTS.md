...
cli/main.py             — CLI entry point (3616 lines). All `ainl` commands.
...
tests/                  — 1056 `*.py` files under `tests/` (~308k lines total); 221 `test_*.py` modules; pytest —collect-only: 1438/1477 (see STATUS.yaml). Definitions: **`STATUS.yaml`** → `real_and_working.tests`.
